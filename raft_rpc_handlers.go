...
func (this *RaftNode) HandleRequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	if this.currentTerm == args.Term && // Vote Request comes from a candidate who has a higher term than us AND
		(this.votedFor == -1 || this.votedFor == args.CandidateId) && // we haven't voted yet AND
		(args.LastLogTerm > nodeLastLogTerm || // Vote Request comes from a candidate who is ATLEAST as up to date as us in terms of log entries
